...
import { mockUser, mockUserRoutines, mockRewards, mockWorkouts } from "@/lib/mock-data";
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { icon: Flame, label: "Racha actual", value: `${mockUser.streak.current} días`, color: "text-orange-500", bg: "bg-orange-50 dark:bg-orange-950/20" },
          { icon: Dumbbell, label: "Ejercicios", value: "12 completados", color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-950/20" },
          { icon: Trophy, label: "Recompensas", value: `${mockRewards.length} ganadas`, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-950/20" },
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-4">
                  <div className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0">
                    <Image src={ur.routine.thumbnail_url} alt={ur.routine.title} fill sizes="64px" className="object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-semibold text-sm">{ur.routine.title}</p>
                      <DifficultyBadge difficulty={ur.routine.difficulty} />
                    </div>
                    <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1">
                      <Clock className="w-3 h-3" /> {ur.routine.duration_minutes} min · {ur.routine.workouts_count} ejercicios
                    </p>
                  ...
                  <Button size="sm" asChild className="bg-amber-500 hover:bg-amber-400 text-white rounded-lg shrink-0">
                    <Link href={`/routines/${ur.routine.id}`}>Continuar</Link>
                  </Button>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="space-y-4">
          ...
          <Card className="gradient-hero text-white border-0">
            <CardContent className="p-5 text-center space-y-2">
              ...
              <p className="text-3xl font-extrabold">{mockUser.streak.current}</p>
              <p className="text-white/80 text-sm">días de racha</p>
              <p className="text-white/50 text-xs">Récord: {mockUser.streak.longest} días</p>
            </CardContent>
          </Card>
