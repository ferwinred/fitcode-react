...
import {
  LogIn, UserPlus, ChevronDown, User, CreditCard,
  Settings, LogOut, Heart, Menu, X, Zap,
} from "lucide-react";
            ...
            <div className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute right-0 top-full mt-1 w-48 bg-[oklch(0.22_0.06_255)] border border-white/10 rounded-xl shadow-xl py-1">
                  <DropdownItem href="/profile" icon={User} label="Perfil" onClick={() => setDropdownOpen(false)} />
                  <DropdownItem href="/favorites" icon={Heart} label="Favoritos" onClick={() => setDropdownOpen(false)} />
                  <DropdownItem href="/pricing" icon={CreditCard} label="Suscripción" onClick={() => setDropdownOpen(false)} />
                  <DropdownItem href="/settings" icon={Settings} label="Configuración" onClick={() => setDropdownOpen(false)} />
      ...
      {menuOpen && (
        <div className="md:hidden bg-[oklch(0.18_0.05_255)] border-t border-white/10 px-4 py-3 space-y-1">
          {navLinks.map((link) => (
            <Link
              ...
              className={cn(
                ...
                pathname === link.href ? "bg-white/10 text-white" : "text-white/70 hover:text-white"
              )}
            >
              {link.label}
            </Link>
          ...
          <div className="pt-2 flex gap-2">
            {isAuthenticated ? (
              <Button variant="ghost" size="sm" onClick={handleLogout} className="flex-1 text-red-400 border border-red-400/30">
                Cerrar sesión
              </Button>
            ) : (
              <>
